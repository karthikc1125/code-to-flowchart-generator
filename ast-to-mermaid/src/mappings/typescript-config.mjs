...
export const typescriptConfig = {
  ...
  // TypeScript: Return ALL top-level statements (includes functions AND other statements)
  // Functions will be processed separately by common-flowchart.mjs
  findStatementNodes(root) {
    if (!root || !root.children) return root?.children || [];
    // Return all top-level children, filtering out syntax tokens
    return root.children.filter(c => c && c.named) || [];
  },
