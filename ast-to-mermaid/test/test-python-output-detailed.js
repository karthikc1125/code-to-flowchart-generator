// Test Python isOutputCall function in detail
...
callNodes.forEach((callNode, index) => {
    ...
    // Test the isOutputCall function step by step
    if (callNode.type === 'call') {
        const callee = (callNode.children || []).find(c => c.named);
        console.log('  Callee:', callee ? callee.type : 'null');
        console.log('  Callee text:', callee ? callee.text : 'null');
        const calleeText = callee ? callee.text : '';
        console.log('  Callee text match print:', /\bprint\b|\boutput\b/i.test(calleeText));
    }
